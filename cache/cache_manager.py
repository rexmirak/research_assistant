...
class CacheManager:
    ...
    def get_metadata(self, paper_id: str) -> Optional[Dict[str, Any]]:
        ...
        cursor.execute(
            "SELECT metadata FROM metadata WHERE paper_id = ?",
            (paper_id,)
        )
        result = cursor.fetchone()
        conn.close()
    ...
    def get_text(self, paper_id: str) -> Optional[tuple[str, str]]:
        ...
        cursor.execute(
            "SELECT text, text_hash FROM text_extracts WHERE paper_id = ?",
            (paper_id,)
        )
        result = cursor.fetchone()
        conn.close()
    ...
    def get_processing_state(self, paper_id: str, stage: str) -> Optional[Dict[str, Any]]:
        ...
        cursor.execute(
            "SELECT state FROM processing_state WHERE paper_id = ? AND stage = ?",
            (paper_id, stage)
        )
        result = cursor.fetchone()
        conn.close()
